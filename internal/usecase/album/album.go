...
func (usecase *albumUsecase) Create(ctx context.Context, album *entity.Album) (*entity.Album, error) {
	...
	// Find all albums
	// albums, err := usecase.albumRepository.GetAllAlbum(ctx, offset, limit)
	// if err != nil {
	// 	return newAlbum, err
	// }
...
func (usecase *albumUsecase) GetAllAlbum(ctx context.Context, offset string, limit string, filter_artist_id string) ([]entity.Album, error) {
	...
	// Get from db
	albums, err := usecase.albumRepository.GetAllAlbum(ctx, offset, limit, filter_artist_id)
	if err != nil {
		return albums, err
	}
